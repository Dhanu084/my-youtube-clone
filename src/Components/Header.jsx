...
const Header = () => {
  ...
  const url = `https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=25&q=${searchQuery}&key=${process.env.REACT_APP_API_KEY}`;
  ...
  useEffect(() => {
    const timer = setTimeout(() => fetchSearchQuery(), 2000);
    return () => {
      clearTimeout(timer);
    };
  }, [searchQuery]);
  ...
  return (
    <div className='grid grid-flow-col h-16 shadow-lg'>
      ...
      <div className='col-span-0 sm:col-span-2'>
        <img
          className='h-12 w-0 sm:w-32 my-2'
